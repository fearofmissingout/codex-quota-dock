...
final class LocalUsageScannerTests: XCTestCase {
    func testScansTokenCountEventsIntoSummaryWindows() throws {
        ...
        XCTAssertEqual(summary.today.effectiveTotal, 11)
        XCTAssertEqual(summary.last7Days.total, 52)
        XCTAssertEqual(summary.last7Days.effectiveTotal, 38)
        XCTAssertEqual(summary.last30Days.total, 52)
        XCTAssertEqual(summary.last30Days.effectiveTotal, 38)
        ...
        XCTAssertEqual(summary.total.uncachedInput, 26)
        XCTAssertEqual(summary.total.effectiveTotal, 38)
        XCTAssertEqual(summary.byDay.map(\.day), ["2026-06-25", "2026-06-30"])
        XCTAssertEqual(summary.byDay.map(\.usage.effectiveTotal), [27, 11])
    }
    ...
    func testScansSQLiteThreadTokenTotals() throws {
        ...
        XCTAssertEqual(summary.sqlite.effectiveTotal, 125)
        XCTAssertEqual(summary.total.total, 125)
        XCTAssertEqual(summary.total.effectiveTotal, 125)
        XCTAssertEqual(summary.today.total, 100)
        XCTAssertEqual(summary.today.effectiveTotal, 100)
